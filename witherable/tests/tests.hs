...
witherableLaws p = testGroup (show (typeRep p))
  ...
  where
    ...
    prop_default_wither s0 f' xs =
        runState (wither f xs) s0 === runState (fmap catMaybes (traverse f xs)) s0
      where
        f a = state $ \s -> applyFun f' (a, s)

    prop_default_witherM :: S -> Fun (A, S) (Maybe B, S) -> f A -> Property
    prop_default_witherM s0 f' xs =
        runState (witherM f xs) s0 === runState (wither f xs) s0
      where
        f a = state $ \s -> applyFun f' (a, s)

    prop_default_filterA :: S -> Fun (A, S) (Bool, S) -> f A -> Property
    prop_default_filterA s0 f' xs =
        runState (filterA f xs) s0 === runState (wither (\a -> (\b -> if b then Just a else Nothing) <$> f a) xs) s0
      where
        f a = state $ \s -> applyFun f' (a, s)
